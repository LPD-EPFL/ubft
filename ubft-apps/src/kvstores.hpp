...
namespace kvstores {
static char const *const libreparent = "DORY_LIB_REPARENT_PATH";
...
static void spawn_memc(int port) {
  ...
  char const *env = getenv(libreparent);
  if (env == nullptr) {
    throw std::runtime_error("Environment variable " + std::string(libreparent) +
                             " not set");
  }

  std::filesystem::path path(env);
  if (!path.is_absolute()) {
    throw std::runtime_error(std::string(env) +
                             " must be an absolute path");
  }


  if (! std::ifstream(env).good()) {
    throw std::runtime_error(std::string(env) +
                             " does not exist");
  }

  std::string const ld_preload = "LD_PRELOAD=" + std::string(env);

  pid_t child_pid = fork();
  if (child_pid == 0) {
    char const *args[] = {"/usr/bin/memcached", "-p",
                          fmt::format("{}", port).c_str(), NULL};
    char const *envs[] = {ld_preload.c_str(),
                          NULL};
...
static size_t put(void *buffer, std::vector<uint8_t> const& key, int const value_size) {
  ...
  std::copy(key.begin(), key.end(), buf + cur);
  cur += key.size();
  ...
  if (cur > put_max_buffer_size(static_cast<int>(key.size()), value_size)) {
...
static size_t get(void *buffer, std::vector<uint8_t> const& key) {
  auto *buf = reinterpret_cast<uint8_t *>(buffer);
  size_t cur = 0;
  buf[cur++] = 'g';
  buf[cur++] = 'e';
  ...
  if (cur > get_max_buffer_size(static_cast<int>(key.size()))) {
    throw std::runtime_error("Buffer overflow");
  }
...
namespace redis {
static void spawn_redis(int port) {
  char const *env = getenv(libreparent);
  if (env == nullptr) {
    throw std::runtime_error("Environment variable " + std::string(libreparent) +
                             " not set");
  ...
  pid_t child_pid = fork();
  if (child_pid == 0) {
    char const *args[] = {"/usr/bin/redis-server", "--save", "\"\"",
                          "--appendonly", "no",
                          "--port", fmt::format("{}", port).c_str(), NULL};
    char const *envs[] = {ld_preload.c_str(),
                          NULL};
    int ret = execve("/usr/bin/redis-server", const_cast<char *const *>(args),
                     const_cast<char *const *>(envs));
    if (ret == -1) {
      throw std::runtime_error(
          fmt::format("Could not start redis: {}", std::strerror(errno)));
    }
  }
}
...
static size_t put(void *buffer, std::vector<uint8_t> const& key, int const value_size) {
  ...
  cur += std::sprintf(reinterpret_cast<char *>(buf + cur), "%d", static_cast<int>(key.size()));
