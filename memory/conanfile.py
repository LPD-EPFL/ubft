...
class DoryMemoryConan(ConanFile):
    name = "dory-memory"
    version = "0.0.1"
    license = "MIT"
    # url = "TODO"
    description = "Physically locked memory with RealTime heartbeat support"
    topics = ("memory", "heartbeat", "rt")
    settings = {
        "os": None,
        "compiler": {
    ...
    def build(self):
        ...
        cmake.definitions["ENABLE_RETAINER"] = self.options.with_retainer

        cmake.configure(source_folder="src")
        cmake.build()

    def package(self):
        self.copy("*.hpp", dst="include/dory/memory", src="src")
        self.copy("*.a", dst="lib", src="lib", keep_path=False)
        self.copy("*.so", dst="lib", src="lib", keep_path=False)

    def package_info(self):
        self.cpp_info.libs = ["dorymemory"]
