 ...
 **/
void async_event_handler(dory::logger const& logger, std::future<void> exit_sig,
  ...
  while (exit_sig.wait_for(std::chrono::seconds(0)) ==
    ...
    if (ret < 0) {
      LOGGER_ERROR(logger, "poll failed");
      return;
    }
    ...
    if (ret) {
      LOGGER_ERROR(logger, "Error, ibv_get_async_event() failed");
      return;
    }

    LOGGER_WARN(logger, "Got async event {}", event.event_type);
