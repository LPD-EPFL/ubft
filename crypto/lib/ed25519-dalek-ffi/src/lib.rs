...
pub extern "C" fn keypair_verify(
    ...
    let sig = Signature::new(*sig_ref);

    match keypair.verify(msg_slice, &sig) {
        Ok(_) => 1,
            _ => 0,
    }
}
...
pub extern "C" fn publickey_verify_raw(
    ...
    let sig = Signature::new(sig_ref.try_into().expect("Incorrect signature length"));

    match public_key.verify_strict(msg_slice, &sig) {
        Ok(_) => 1,
            _ => 0,
    }
}
