...
void mica_init(struct mica_kv* kv, int instance_id, int num_bkts,
               int log_cap, uint8_t* log) {
  int i, j;
  ...
  hrd_red_printf(
      ...
      "Buckets = %d (size = %u B), log capacity = %d B.\n",
      instance_id, num_bkts, num_bkts * sizeof(struct mica_bkt),
      log_cap);
  ...
  printf("mica: Allocating hash table index for instance %d\n", instance_id);
  kv->ht_index = (struct mica_bkt*)malloc(num_bkts * sizeof(struct mica_bkt));

  for (i = 0; i < num_bkts; i++) {
    for (j = 0; j < 8; j++) {
      kv->ht_index[i].slots[j].in_use = 0;
      kv->ht_index[i].slots[j].tag = 0;
    }
  }
...
void mica_free(struct mica_kv* kv) {
  free(kv->ht_log);
  free(kv->ht_index);
}
...
void mica_single_op(struct mica_kv* kv, struct mica_op* op,
                   struct mica_resp* resp) {
  int n = 1;
  struct mica_op* op_arr[1];
  op_arr[0] = op;

  mica_batch_op(kv, n, op_arr, resp);
}
