...
int main(int argc, char *argv[]) {
  ...
  std::vector<int> const hosts_ids = {1, 2, 3};
  ...
  // Everybody is a host
  if (std::find(hosts_ids.begin(), hosts_ids.end(), local_id) ==
      hosts_ids.end()) {
    throw std::runtime_error(fmt::format("Unknown id {}.", local_id));
  }
  dory::ubft::replicated_swmr::HostBuilder host_builder(
      cb, local_id, writer_id, {reader_id, writer_id}, "main", nb_registers,
      register_size);
  host_builder.announceQps();

  if (local_id == writer_id) {
    dory::ubft::replicated_swmr::WriterBuilder builder(
        cb, writer_id, hosts_ids, "main", nb_registers, register_size, true);

    builder.announceQps();
    store.barrier("qp_announced", 3);
    host_builder.connectQps();
    builder.connectQps();
    ...
    dory::ubft::replicated_swmr::ReaderBuilder builder(
        cb, local_id, writer_id, hosts_ids, "main", nb_registers,
        register_size);
    ...
    for (size_t i = 0; i < nb_reads; i++) {
      ...
      std::vector<uint8_t> words{
          opt_completion->first.get(),
          opt_completion->first.get() + std::min(register_size, 10UL)};
