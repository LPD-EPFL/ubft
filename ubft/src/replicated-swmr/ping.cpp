...
int main(int argc, char *argv[]) {
  ...
  std::vector<int> hosts_ids{measurer_id, responder_id, 3};
  ...
  dory::ubft::replicated_swmr::HostBuilder ping_host_builder(
      cb, local_id, measurer_id, {measurer_id, responder_id}, "ping",
      nb_registers, register_size);
  dory::ubft::replicated_swmr::HostBuilder pong_host_builder(
      cb, local_id, responder_id, {measurer_id, responder_id}, "pong",
      nb_registers, register_size);
  ping_host_builder.announceQps();
  pong_host_builder.announceQps();

  if (local_id == measurer_id) {
    dory::ubft::replicated_swmr::WriterBuilder ping_builder(
        cb, local_id, hosts_ids, "ping", nb_registers, register_size, true);
    dory::ubft::replicated_swmr::ReaderBuilder pong_builder(
        cb, local_id, responder_id, hosts_ids, "pong", nb_registers,
        register_size);

    ping_builder.announceQps();
    pong_builder.announceQps();
    store.barrier("qp_announced", 3);
    ping_host_builder.connectQps();
    pong_host_builder.connectQps();
    ...
    for (size_t e = 0; e < experiments; e++) {
      ...
      for (size_t p = 0; p < pings; p++) {
        ...
        while (!ponged) {
          ...
          dory::ubft::replicated_swmr::Reader::PollResult opt_polled;
    ...
    dory::ubft::replicated_swmr::ReaderBuilder ping_builder(
        cb, local_id, measurer_id, hosts_ids, "ping", nb_registers,
        register_size);
    dory::ubft::replicated_swmr::WriterBuilder pong_builder(
        cb, local_id, hosts_ids, "pong", nb_registers, register_size, true);
    ...
    for (size_t write_nb = 0; write_nb < experiments * pings; write_nb++) {
      ...
      while (!pinged) {
        ...
        dory::ubft::replicated_swmr::Reader::PollResult polled;
